    ...
    module Helpers
      ...
      def remember_device
        cookies.signed[:remember_device] = {
            ...
            :expires => resource_class.duo_remember_device.from_now
        }
      end
      ...
      def require_token?
        if cookies.signed[:remember_device].present? &&
          ...
          resource_class.duo_remember_device.to_i
          return false
        end
      ...
      def check_request_and_redirect_to_verify_token
        if signed_in?(resource_name) &&
            warden.session(resource_name)[:with_duo_authentication] &&
            require_token?
          # login with 2fa
      ...
      def verify_duo_path_for(resource_or_scope = nil)
        scope = Devise::Mapping.find_scope!(resource_or_scope)
        send(:"#{scope}_verify_duo_path")
      end
    end
  end
end